...
function compilePlugins(workingDir,oringPlugins){
    let flattenedPlugins = []
    let trace = []
    oringPlugins.forEach((plugin)=>{
      recursiveCompileSinglePlugin(workingDir,plugin,trace,flattenedPlugins)
    })
...
function requireNpmModule(path,workingDir){
    if(path.startsWith(".")){
       return requireRelativePlugin(workingDir,path)
    }
    else {
      return require(path)
    }
...
function createPluginFronFactory(workingDir,factory,options){
    if(_.isFunction(factory)){
       return factory(options)
    }
    else if(_.isString(factory)){
       return requireNpmModule(factory,workingDir)(options)
    }
    else {
      assert(false,`plugin factory should be string or function, not ${factory}`)
    }
}

function recursiveCompileSinglePlugin(workingDir, plugin, trace, flattenedPlugins){
    ...
    if(Array.isArray(plugin)){
      ...
      for (let subplugin of plugin) {
        recursiveCompileSinglePlugin(workingDir, subplugin, trace,flattenedPlugins)
      }
      trace.pop()
    }
    else {
      ...
      if(typeof plugin === "string"){
          plugin = requireNpmModule(plugin,workingDir)
          recursiveCompileSinglePlugin(workingDir, plugin, trace,flattenedPlugins)
      }
      else {
        ...
        // factory + options
          ...
          plugin = createPluginFronFactory(workingDir,plugin.plugin,plugin.options)
          plugin._debug = {
            trace:_.clone(trace)
          }
          flattenedPlugins.push(plugin)
          ...
          assert(false,`can't recognize the type of plugin: ${plugin.info}`)
        }
 ...
 class Manager {
  constructor(options) {
    ...
    assert(options.workingDir, "workingDir for Manager is required to load your plugins")
    this._workingDir = options.workingDir
    this.plugins = compilePlugins(this._workingDir,options.plugins)
