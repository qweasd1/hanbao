...
function assert(isTrue,message){
    if(!isTrue){
      let error = new HanbaoError(message)
 ...
 class Manager {
   ...
   /**
    *
    * @param options  utils, workingDir[required], plugins[required]
    */
  constructor(options) {
    ...
    this.meta("workingDir",options.workingDir)
    this.plugins = helper.loadHanbaoModule(options.plugins, {
      trace:[],
      workingDir:this.meta("workingDir"),
      check:(plugin)=>{
        if(!plugin.info){
            return "plugin should have 'info' attribute to store meta data"
        }
        
        if(typeof plugin.info !== 'object'){
            return "plugin's 'info' attribute should be object"
        }
        
        if(!plugin.info.name ){
           return "plugin's 'info' attribute should have 'name' attribute"
        }
    ...
    // check the uniqueness of plugin name, if duplicate, throw exception
    this._assertNoDuplicatePlugins()
 ...
 _processSingleLayerModel(processor,next){
  ...
  _assertNoDuplicatePlugins(){
    let pluginMap = new Map()
    for (let p of this.plugins) {
      if(!pluginMap.has(p.info.name)){
          pluginMap.set(p.info.name,p)
      }
      else {
        helper.raise(new HanbaoError("2 plugins has the same name",[pluginMap.get(p.info.name),p]))
      }
    }
  }
}
